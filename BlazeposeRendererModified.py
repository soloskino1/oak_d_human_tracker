...
class BlazeposeRenderer:
    ...
    #Added condition to only draw xyz if draw_xyz is true
    def draw_landmarks(self, body, draw_xyz):
        ...
        if self.show_xyz and body.xyz_ref and draw_xyz:
            try:
    ...
    def draw(self, frame, body, draw_xyz):
        if not self.pause:
            self.frame = frame
            if body:
                self.draw_landmarks(body, draw_xyz)
            self.body = body
