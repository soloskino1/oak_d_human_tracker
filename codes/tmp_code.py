...
while True:
    ...
    rr.center.y    = (sqn_rr_center_y * 1792 - 392) / 1008
    rr.size.width  = sqn_rr_size
    rr.size.height = sqn_rr_size * 1792 / 1008
    ...
    if lm_score > 0.4:
        ...
        xyz_zone = 0
        xyz_ref = 0
        if is_visible(right_hip) and is_visible(left_hip) and is_visible(right_shoulder) and is_visible(left_shoulder):
            kp1 = right_shoulder
            rrn_xyz_ref_x, rrn_xyz_ref_y = (lms[5*kp1] / 256, lms[5*kp1+1] / 256) 
            sqn_xyz_ref_x, sqn_xyz_ref_y = rr2img(rrn_xyz_ref_x, rrn_xyz_ref_y)
            half_zone_size = max(int(sqn_rr_size * 1792 / 90), 4)
            xc = filter_x.apply(sqn_xyz_ref_x * 1792 + 0)
            yc = filter_y.apply(sqn_xyz_ref_y * 1792 - 392)
            roi_left = max(0, xc - half_zone_size)
            roi_top = max(0, yc - half_zone_size)
            kp1 = left_shoulder
            kp2 = left_hip
            rrn_xyz_ref_x, rrn_xyz_ref_y = (lms[5*kp1] / 256, lms[5*kp1+1] / 256) 
            rrn_xyz_ref_x2, rrn_xyz_ref_y2 = (lms[5*kp2] / 256, lms[5*kp2+1] / 256) 
            sqn_xyz_ref_x, sqn_xyz_ref_y = rr2img(rrn_xyz_ref_x, rrn_xyz_ref_y)
            sqn_xyz_ref_x2, sqn_xyz_ref_y2 = rr2img(rrn_xyz_ref_x2, rrn_xyz_ref_y2)
            half_zone_size = max(int(sqn_rr_size * 1792 / 90), 4)
            xc = filter_x.apply(sqn_xyz_ref_x * 1792 + 0)
            yc = filter_y.apply(sqn_xyz_ref_y2 * 1792 - 392)  
            roi_right = min(1792-1, xc + half_zone_size)
            roi_bottom = min(1008-1, yc + half_zone_size)
            xyz_ref = 1  
